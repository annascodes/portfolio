...
const ContactFormModal = ({onlyIcon=false}:{onlyIcon?:boolean}) => {
    ...
    return (
        <div>
            ...
            <button className={`btn  tracking-widest btn-outline text-xs flex items-center gap-2 border-yellow-300 ${onlyIcon && 'border-none'} `}
                onClick={() => {
                    const modal = document.getElementById(modalId) as HTMLDialogElement | null;
                    modal?.showModal()
                }}> <BasicIcons label='hand' className='text-xl text-yellow-300' /> {!onlyIcon &&'Say hi to Anas' }  </button>
